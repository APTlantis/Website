...
func (v vector) testSign(t *testing.T) {
	private := NewKeyFromSeed(v.sk)

	var got []byte

	if v.ph {
		got = SignPh(private, v.msg, string(v.ctx))
	} else {
		got = Sign(private, v.msg, string(v.ctx))
	}
...
func (v vector) testVerify(t *testing.T) {
	...
	if v.ph {
		got = VerifyPh(v.pk, v.msg, v.sig, string(v.ctx))
	} else {
		got = Verify(v.pk, v.msg, v.sig, string(v.ctx))
	}
