...
func TestVectors(t *testing.T) {
	...
	for _, name := range []string{"P256", "P521"} {
		t.Run(name+"/G1", func(t *testing.T) { testVector[KeyG2SigG1](t, "g1", name) })
		t.Run(name+"/G2", func(t *testing.T) { testVector[KeyG1SigG2](t, "g2", name) })
	}
}

func testVector[K KeyGroup](t *testing.T, group, name string) {
	...
	for _, f := range zipFile.File {
		...
		for scanner.Scan() {
			...
			keyInfo := []byte("")
			priv, err := KeyGen[K](seed, salt, keyInfo)
			...
			pub := priv.PublicKey()
			test.CheckOk(Verify(pub, msg, sig), "cannot verify", t)
		}
	}
}
