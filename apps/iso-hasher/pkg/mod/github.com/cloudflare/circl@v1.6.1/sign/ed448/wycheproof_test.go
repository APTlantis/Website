...
func (kat *Wycheproof) keyPair(t *testing.T) {
	for i, g := range kat.Groups {
		...
		keys := NewKeyFromSeed(private)
		got := keys.Public().(PublicKey)
		want := public
...
func (kat *Wycheproof) verify(t *testing.T) {
	...
	for i, g := range kat.Groups {
		for _, gT := range g.Tests {
			...
			priv := NewKeyFromSeed(private)
			got := priv.Public().(PublicKey)
			want := public
			if !bytes.Equal(got, want) {
				test.ReportError(t, got, want, i, gT.TcID)
			}
			if isValid {
				got := Sign(priv, msg, string(ctx))
				want := sig
				if !bytes.Equal(got, want) {
					test.ReportError(t, got, want, i, gT.TcID)
				}
			...
			{
				got := Verify(priv.Public().(PublicKey), msg, sig, string(ctx))
				want := isValid
				if got != want {
					test.ReportError(t, got, want, i, gT.TcID)
				}
