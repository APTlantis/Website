...
func TestSigner(t *testing.T) {
	...
	for _, o := range []SignerOptions{
		{Scheme: ED448, Hash: crypto.Hash(0), Context: ""},
		{Scheme: ED448, Hash: crypto.Hash(0), Context: "non-empty"},
		{Scheme: ED448Ph, Hash: crypto.Hash(0), Context: ""},
		{Scheme: ED448Ph, Hash: crypto.Hash(0), Context: "non-empty"},
	} {
		testSigner(t, key, o)
	}
}
...
func TestErrors(t *testing.T) {
	...
	t.Run("bigContext", func(t *testing.T) {
		...
		var want error
		_, priv, _ := GenerateKey(nil)
		got := test.CheckPanic(func() { Sign(priv, msg[:], string(ctx[:])) })
		if got != want {
			test.ReportError(t, got, want)
		}
	})
...
func BenchmarkVerification(b *testing.B) {
	...
	message := []byte("Hello, world!")
	ctx := "a context string"
	signature := Sign(priv, message, ctx)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Verify(pub, message, signature, ctx)
	}
}
...
func Example_ed448() {
	...
	ok := Verify(pub, message, signature, ctx)
	fmt.Println(ok)
	// Output: true
}
