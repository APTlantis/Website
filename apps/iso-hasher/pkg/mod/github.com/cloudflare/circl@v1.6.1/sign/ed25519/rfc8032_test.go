...
func (v *rfc8032Vector) fetch(line string) {
	...
	v.private = v.private[:SeedSize]
	v.signature = v.signature[:SignatureSize]
}

func (v *rfc8032Vector) test(t *testing.T, lineNum int) {
	key := NewKeyFromSeed(v.private)
	{
		got := key.Public().(PublicKey)
		want := v.public
		if !bytes.Equal(got, want) {
			test.ReportError(t, got, want, lineNum, v)
		}

		got = Sign(key, v.message)
		want = v.signature
		if !bytes.Equal(got, want) {
			test.ReportError(t, got, want, lineNum, v)
		}
	...
	{
		got := Verify(key.Public().(PublicKey), v.message, v.signature)
		want := true
		if got != want {
			test.ReportError(t, got, want, lineNum, v)
		}
...
func (v vector) testSign(t *testing.T) {
	key := NewKeyFromSeed(v.sk)

	var got []byte

	if v.ph {
		got = SignPh(key, v.msg, "")
	} else if v.ctxLen > 0 {
		got = SignWithCtx(key, v.msg, string(v.ctx))
	} else {
		got = Sign(key, v.msg)
	}
...
func (v vector) testVerify(t *testing.T) {
	var got bool
	if v.ph {
		got = VerifyPh(v.pk, v.msg, v.sig, "")
	} else if v.ctxLen > 0 {
		got = VerifyWithCtx(v.pk, v.msg, v.sig, string(v.ctx))
	} else {
		got = Verify(v.pk, v.msg, v.sig)
	}
