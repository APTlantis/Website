...
func testExp(t *testing.T, m Encoder) {
	const testTimes = 1 << 8
	params := m.GetParams()
	TBytes := (params.T + 7) / 8
...
type zzAdd struct{ set Params }

func (zzAdd) Identity() EltG { return big.NewInt(0) }
func (zzAdd) NewEltP() EltP  { return new(big.Int) }
func (zzAdd) Sqr(x EltG) {
	a := x.(*big.Int)
	a.Add(a, a)
}

func (zzAdd) Mul(x EltG, y EltP) {
	a := x.(*big.Int)
	b := y.(*big.Int)
	a.Add(a, b)
}

func (z zzAdd) ExtendedEltP() EltP {
	a := big.NewInt(1)
	a.Lsh(a, z.set.W*z.set.D)
	return a
}

func (z zzAdd) Lookup(x EltP, idTable uint, sgnElt int32, idElt int32) {
	a := x.(*big.Int)
	a.SetInt64(1)
