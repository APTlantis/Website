...
func TestLagrange(t *testing.T) {
	...
	err := test.CheckPanic(func() { NewLagrangePolynomial(x, y[:1]) })
	test.CheckNoErr(t, err, "should panic")

	// Test that nodes must be different.
	x[0].Set(x[1])
	err = test.CheckPanic(func() { NewLagrangePolynomial(x, y) })
	test.CheckNoErr(t, err, "should panic")

	// Test LagrangeBase wrong index
	err = test.CheckPanic(func() { LagrangeBase(10, x, zero) })
	test.CheckNoErr(t, err, "should panic")
}
