...
func TestAscon(t *testing.T) {
	...
	for _, mode := range []Mode{Ascon128, Ascon128a, Ascon80pq} {
		name := mode.String()
		t.Run(name, func(t *testing.T) {
			vectors := readFile(t, "testdata/"+name+".json")
			for _, v := range vectors {
				a, err := New(v.Key, mode)
...
func benchmarkSeal(b *testing.B, buf []byte, mode Mode) {
	b.ReportAllocs()
	b.SetBytes(int64(len(buf)))

	var key []byte
	switch mode {
	case Ascon128, Ascon128a:
...
func benchmarkOpen(b *testing.B, buf []byte, mode Mode) {
	b.ReportAllocs()
	b.SetBytes(int64(len(buf)))
