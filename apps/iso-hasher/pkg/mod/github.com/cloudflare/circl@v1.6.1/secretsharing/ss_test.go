...
func TestSecretSharing(tt *testing.T) {
	...
	tt.Run("verifyShares", func(ttt *testing.T) {
		for i := range shares {
			test.CheckOk(Verify(t, shares[i], coms) == true, "failed one share", ttt)
		}
	})
	...
	tt.Run("badShares", func(ttt *testing.T) {
		...
		for i := range badShares {
			test.CheckOk(Verify(t, badShares[i], coms) == false, "verify must fail due to bad shares", ttt)
		}
	})

	tt.Run("badCommitments", func(ttt *testing.T) {
		badComs := make(SecretCommitment, len(coms))
		for i := range coms {
