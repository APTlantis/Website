...
func testExpander(t *testing.T, vs *vectorExpanderSuite) {
	...
	switch vs.Hash {
	case "SHA256":
		exp = NewExpanderMD(crypto.SHA256, []byte(vs.DST))
	case "SHA512":
		exp = NewExpanderMD(crypto.SHA512, []byte(vs.DST))
	case "SHAKE128":
		exp = NewExpanderXOF(xof.SHAKE128, vs.K, []byte(vs.DST))
	case "SHAKE256":
		exp = NewExpanderXOF(xof.SHAKE256, vs.K, []byte(vs.DST))
	default:
...
func BenchmarkExpander(b *testing.B) {
	...
	for _, v := range []struct {
		...
		{"XMD", NewExpanderMD(crypto.SHA256, dst)},
		{"XOF", NewExpanderXOF(xof.SHAKE128, 0, dst)},
	} {
		exp := v.Exp
		for l := 8; l <= 10; l++ {
			max := int64(1) << uint(l)
