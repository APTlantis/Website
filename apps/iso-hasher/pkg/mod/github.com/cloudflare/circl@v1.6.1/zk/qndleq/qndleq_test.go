...
func TestProve(t *testing.T) {
	...
	for i := 0; i < testTimes; i++ {
		...
		proof, err := Prove(rand.Reader, x, g, gx, h, hx, N, SecParam)
...
func Benchmark_qndleq(b *testing.B) {
	...
	x, _ := rand.Int(rand.Reader, N)
	g, _ := SampleQn(rand.Reader, N)
	h, _ := SampleQn(rand.Reader, N)
	gx := new(big.Int).Exp(g, x, N)
	hx := new(big.Int).Exp(h, x, N)

	proof, _ := Prove(rand.Reader, x, g, gx, h, hx, N, SecParam)

	b.Run("Prove", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = Prove(rand.Reader, x, g, gx, h, hx, N, SecParam)
		}
	})
