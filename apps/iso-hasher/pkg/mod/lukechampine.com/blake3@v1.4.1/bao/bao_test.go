...
func TestBaoGolden(t *testing.T) {
	...
	interleaved, root := EncodeBuf(data, 0, false)
	...
	outboard, root := EncodeBuf(data, 0, true)
	if toHex(root[:]) != "6654fbd1836b531b25e2782c9cc9b792c80abb36b024f59db5d5f6bd3187ddfe" {
		t.Errorf("bad root: %x", root)
	...
	// test empty input
	interleaved, root = EncodeBuf(nil, 0, false)
	...
	} else if !VerifyBuf(interleaved, nil, 0, root) {
		t.Error("verify failed")
	}
	outboard, root = EncodeBuf(nil, 0, true)
	if toHex(root[:]) != "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262" {
		t.Errorf("bad root: %x", root)
	} else if toHex(outboard[:]) != "0000000000000000" {
		t.Errorf("bad outboard encoding: %x", outboard)
	} else if !VerifyBuf(nil, outboard, 0, root) {
		t.Error("verify failed")
	}
}
...
func TestBaoOutboard(t *testing.T) {
	...
	for group := 0; group < 10; group++ {
		outboard, root := EncodeBuf(data, group, true)
		if !VerifyBuf(data, outboard, group, root) {
			t.Fatal("verify failed")
		}
		badRoot := root
		badRoot[0] ^= 1
		if VerifyBuf(data, outboard, group, badRoot) {
			t.Fatal("verify succeeded with bad root")
		}
		badPrefix := append([]byte(nil), outboard...)
		badPrefix[0] ^= 1
		if VerifyBuf(data, badPrefix, group, root) {
			t.Fatal("verify succeeded with bad length prefix")
		}
		badCVs := append([]byte(nil), outboard...)
		badCVs[8] ^= 1
		if VerifyBuf(data, badCVs, group, root) {
			t.Fatal("verify succeeded with bad cv data")
		}
	}
}
...
func TestBaoChunkGroup(t *testing.T) {
	...
	for _, test := range []struct {
		...
		_, root := EncodeBuf(input, group, false)
...
func TestBaoStreaming(t *testing.T) {
	...
	enc, root := EncodeBuf(data, 0, false)
	if root != blake3.Sum256(data) {
		t.Fatal("bad root")
	}
	var buf bytes.Buffer
	if ok, err := Decode(&buf, bytes.NewReader(enc), nil, 0, root); err != nil || !ok {
	...
	buf.Reset()
	if ok, err := Decode(&buf, bytes.NewReader(enc), nil, 0, [32]byte{}); err != nil {
		t.Fatal("decode failed")
	} else if ok {
	...
	buf.Reset()
	enc[len(enc)/2] ^= 1
	if ok, err := Decode(&buf, bytes.NewReader(enc), nil, 0, root); err != nil {
		t.Fatal("decode failed")
	} else if ok {
...
func TestBaoSlice(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// combined encoding
		{
			enc, root := EncodeBuf(data, 0, false)
			var buf bytes.Buffer
			if err := ExtractSlice(&buf, bytes.NewReader(enc), nil, 0, test.off, test.len); err != nil {
				t.Error(err)
			} else if vdata, ok := VerifySlice(buf.Bytes(), 0, test.off, test.len, root); !ok {
		...
		// outboard encoding
		{
			enc, root := EncodeBuf(data, 0, true)
			start, end := (test.off/1024)*1024, ((test.off+test.len+1024-1)/1024)*1024
			if end > uint64(len(data)) {
				end = uint64(len(data))
			}
			var buf bytes.Buffer
			if err := ExtractSlice(&buf, bytes.NewReader(data[start:end]), bytes.NewReader(enc), 0, test.off, test.len); err != nil {
				t.Error(err)
			} else if vdata, ok := VerifySlice(buf.Bytes(), 0, test.off, test.len, root); !ok {
