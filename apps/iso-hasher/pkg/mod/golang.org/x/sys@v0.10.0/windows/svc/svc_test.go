...
func getState(t *testing.T, s *mgr.Service) State {
	status, err := s.Query()
	if err != nil {
...
func testState(t *testing.T, s *mgr.Service, want State) {
	have := getState(t, s)
	if have != want {
...
func waitState(t *testing.T, s *mgr.Service, want State) {
	for i := 0; ; i++ {
		have := getState(t, s)
		if have == want {
			return
...
func TestIsAnInteractiveSession(t *testing.T) {
	isInteractive, err := IsAnInteractiveSession()
	if err != nil {
		t.Fatal(err)
	}
	if !isInteractive {
