...
func TestWin32finddata(t *testing.T) {
	...
	pathp, _ := UTF16PtrFromString(path)
	h, err := FindFirstFile(pathp, &(x.fd))
	if err != nil {
...
func TestFormatMessage(t *testing.T) {
	...
	buf := make([]uint16, 300)
	_, err := FormatMessage(flags, uintptr(dll.Handle), TITLE_SC_MESSAGE_BOX, 0, buf, nil)
	if err != nil {
...
func ExampleLoadLibrary() {
	h, err := LoadLibrary("kernel32.dll")
	if err != nil {
		abort("LoadLibrary", err)
	}
	defer FreeLibrary(h)
	proc, err := GetProcAddress(h, "GetVersion")
	if err != nil {
...
func TestTOKEN_ALL_ACCESS(t *testing.T) {
	if TOKEN_ALL_ACCESS != 0xF01FF {
		t.Errorf("TOKEN_ALL_ACCESS = %x, want 0xF01FF", TOKEN_ALL_ACCESS)
	}
}

func TestCreateWellKnownSid(t *testing.T) {
	sid, err := CreateWellKnownSid(WinBuiltinAdministratorsSid)
	if err != nil {
...
func TestPseudoTokens(t *testing.T) {
	...
	realProcessToken, err := OpenCurrentProcessToken()
	if err != nil {
		t.Fatal(err)
	}
	...
	pseudoProcessToken := GetCurrentProcessToken()
	pseudoProcessUser, err := pseudoProcessToken.GetTokenUser()
	if err != nil {
		t.Fatal(err)
	}
	if !EqualSid(realProcessUser.User.Sid, pseudoProcessUser.User.Sid) {
	...
	pseudoThreadToken := GetCurrentThreadToken()
	_, err = pseudoThreadToken.GetTokenUser()
	if err != ERROR_NO_TOKEN {
		t.Fatal("Expected an empty thread token")
	}
	pseudoThreadEffectiveToken := GetCurrentThreadEffectiveToken()
	pseudoThreadEffectiveUser, err := pseudoThreadEffectiveToken.GetTokenUser()
	if err != nil {
		t.Fatal(nil)
	}
	if !EqualSid(realProcessUser.User.Sid, pseudoThreadEffectiveUser.User.Sid) {
	...
	if !EqualSid(realProcessUser.User.Sid, pseudoThreadUser.User.Sid) {
...
func TestGUID(t *testing.T) {
	...
	_, err = GUIDFromString("not-a-real-guid")
	if err != syscall.Errno(CO_E_CLASSSTRING) {
...
func TestKnownFolderPath(t *testing.T) {
	...
	got, err := KnownFolderPath(FOLDERID_Desktop, KF_FLAG_DEFAULT)
	if err != nil {
		t.Fatal(err)
	}
	if want != got {
...
func TestGetNamedSecurityInfo(t *testing.T) {
	path, err := GetSystemDirectory()
	if err != nil {
		t.Fatal(err)
	}
	sd, err := GetNamedSecurityInfo(path, SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION)
	if err != nil {
		t.Fatal(err)
	}
	if !sd.IsValid() {
...
func TestGetSecurityInfo(t *testing.T) {
	sd, err := GetSecurityInfo(CurrentProcess(), SE_KERNEL_OBJECT, DACL_SECURITY_INFORMATION)
	if err != nil {
		t.Fatal(err)
	}
	if !sd.IsValid() {
...
func TestSddlConversion(t *testing.T) {
	sd, err := SecurityDescriptorFromString("O:BA")
	if err != nil {
		t.Fatal(err)
	}
	if !sd.IsValid() {
	...
	if !sdOwner.IsWellKnown(WinBuiltinAdministratorsSid) {
...
func TestBuildSecurityDescriptor(t *testing.T) {
	...
	adminSid, err := CreateWellKnownSid(WinBuiltinAdministratorsSid)
	if err != nil {
		t.Fatal(err)
	}
	systemSid, err := CreateWellKnownSid(WinLocalSystemSid)
	if err != nil {
		t.Fatal(err)
	}
	...
	access := []EXPLICIT_ACCESS{{
		...
		Trustee: TRUSTEE{
			...
			TrusteeValue: TrusteeValueFromSID(adminSid),
		},
	}}
	owner := &TRUSTEE{
		TrusteeForm:  TRUSTEE_IS_SID,
		TrusteeType:  TRUSTEE_IS_USER,
		TrusteeValue: TrusteeValueFromSID(systemSid),
	}

	sd, err := BuildSecurityDescriptor(owner, nil, access, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	sd, err = NewSecurityDescriptor()
	if err != nil {
		t.Fatal(err)
	}
	acl, err := ACLFromEntries(access, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetDiskFreeSpaceEx(t *testing.T) {
	cwd, err := UTF16PtrFromString(".")
	if err != nil {
		t.Fatalf(`failed to call UTF16PtrFromString("."): %v`, err)
	}
	var freeBytesAvailableToCaller, totalNumberOfBytes, totalNumberOfFreeBytes uint64
	if err := GetDiskFreeSpaceEx(cwd, &freeBytesAvailableToCaller, &totalNumberOfBytes, &totalNumberOfFreeBytes); err != nil {
...
func TestProcessWorkingSetSizeEx(t *testing.T) {
	...
	var flag uint32
	GetProcessWorkingSetSizeEx(hProcess, &minimumWorkingSetSize, &maximumWorkingSetSize, &flag)

	// Set the new limits to the current ones
	if err := SetProcessWorkingSetSizeEx(hProcess, minimumWorkingSetSize, maximumWorkingSetSize, flag); err != nil {
		t.Error(err)
	}
}

func TestJobObjectInfo(t *testing.T) {
	jo, err := CreateJobObject(nil, nil)
	if err != nil {
		t.Fatalf("CreateJobObject failed: %v", err)
	}
	defer CloseHandle(jo)

	var info JOBOBJECT_EXTENDED_LIMIT_INFORMATION

	err = QueryInformationJobObject(jo, JobObjectExtendedLimitInformation,
	...
	_, err = SetInformationJobObject(jo, JobObjectExtendedLimitInformation,
...
func TestIsWow64Process2(t *testing.T) {
	...
	err := IsWow64Process2(CurrentProcess(), &processMachine, &nativeMachine)
	if errors.Is(err, ERROR_PROC_NOT_FOUND) {
		maj, min, build := RtlGetNtVersionNumbers()
...
func TestNTStatusConversion(t *testing.T) {
	want := ERROR_TOO_MANY_NAMES
	got := STATUS_TOO_MANY_NAMES.Errno()
	if want != got {
...
func TestPEBFilePath(t *testing.T) {
	...
	for cur := peb.Ldr.InMemoryOrderModuleList.Flink; cur != &peb.Ldr.InMemoryOrderModuleList; cur = cur.Flink {
		e := (*LDR_DATA_TABLE_ENTRY)(unsafe.Pointer(uintptr(unsafe.Pointer(cur)) - unsafe.Offsetof(LDR_DATA_TABLE_ENTRY{}.InMemoryOrderLinks)))
...
func TestResourceExtraction(t *testing.T) {
	system32, err := GetSystemDirectory()
	if err != nil {
		t.Errorf("unable to find system32 directory: %v", err)
	}
	cmd, err := LoadLibrary(filepath.Join(system32, "cmd.exe"))
	if err != nil {
		t.Errorf("unable to load cmd.exe: %v", err)
	}
	defer FreeLibrary(cmd)
	rsrc, err := FindResource(cmd, CREATEPROCESS_MANIFEST_RESOURCE_ID, RT_MANIFEST)
	if err != nil {
		t.Errorf("unable to find cmd.exe manifest resource: %v", err)
	}
	manifest, err := LoadResourceData(cmd, rsrc)
	if err != nil {
...
func TestCommandLineRecomposition(t *testing.T) {
	...
	for i := 0; i < trials; i++ {
		...
		commandLine := ComposeCommandLine(args)
		decomposedArgs, err := DecomposeCommandLine(commandLine)
		if err != nil {
	...
	for _, test := range testsWithNUL {
		_, err := DecomposeCommandLine(test)
		if err == nil {
...
func TestWinVerifyTrust(t *testing.T) {
	...
	evsignedfile16, err := UTF16PtrFromString(evsignedfile)
	if err != nil {
	...
	data := &WinTrustData{
		...
		RevocationChecks: WTD_REVOKE_NONE, // No revocation checking, in case the tests don't have network connectivity.
		UnionChoice:      WTD_CHOICE_FILE,
		StateAction:      WTD_STATEACTION_VERIFY,
		FileOrCatalogOrBlobOrSgnrOrCert: unsafe.Pointer(&WinTrustFileInfo{
			Size:     uint32(unsafe.Sizeof(WinTrustFileInfo{})),
			FilePath: evsignedfile16,
		}),
	}
	verifyErr := WinVerifyTrustEx(InvalidHWND, &WINTRUST_ACTION_GENERIC_VERIFY_V2, data)
	data.StateAction = WTD_STATEACTION_CLOSE
	closeErr := WinVerifyTrustEx(InvalidHWND, &WINTRUST_ACTION_GENERIC_VERIFY_V2, data)
	if verifyErr != nil {
	...
	evsignedfile16, err = UTF16PtrFromString(corruptedEvsignedfile)
	if err != nil {
	...
	data.StateAction = WTD_STATEACTION_CLOSE
	closeErr = WinVerifyTrustEx(InvalidHWND, &WINTRUST_ACTION_GENERIC_VERIFY_V2, data)
	if verifyErr != Errno(TRUST_E_BAD_DIGEST) {
...
func TestEnumProcesses(t *testing.T) {
	...
	err := EnumProcesses(pids[:], &outSize)
	if err != nil {
...
func TestProcessModules(t *testing.T) {
	...
	var module Handle
	var cbNeeded uint32
	err = EnumProcessModules(process, &module, uint32(unsafe.Sizeof(module)), &cbNeeded)
	if err != nil {
		t.Fatalf("EnumProcessModules failed: %v", err)
	}

	var moduleEx Handle
	err = EnumProcessModulesEx(process, &moduleEx, uint32(unsafe.Sizeof(moduleEx)), &cbNeeded, LIST_MODULES_DEFAULT)
	if err != nil {
	...
	err = GetModuleFileNameEx(process, module, &modulePathUTF16[0], uint32(len(modulePathUTF16)))
	if err != nil {
		t.Fatalf("GetModuleFileNameEx failed: %v", err)
	}

	modulePath := UTF16ToString(modulePathUTF16)
	if modulePath != exePath {
		t.Fatalf("module does not match executable for GetModuleFileNameEx: %s != %s", modulePath, exePath)
	}

	err = GetModuleBaseName(process, module, &modulePathUTF16[0], uint32(len(modulePathUTF16)))
	if err != nil {
	...
	err = GetModuleInformation(process, module, &moduleInfo, uint32(unsafe.Sizeof(moduleInfo)))
	if err != nil {
...
func TestQueryWorkingSetEx(t *testing.T) {
	...
	information := PSAPI_WORKING_SET_EX_INFORMATION{
		VirtualAddress: Pointer(unsafe.Pointer(&a)),
	}
	infos := []PSAPI_WORKING_SET_EX_INFORMATION{information}

	cb := uint32(uintptr(len(infos)) * unsafe.Sizeof(infos[0]))
	if err := QueryWorkingSetEx(process, uintptr(unsafe.Pointer(&infos[0])), cb); err != nil {
		t.Fatalf("%+v", err)
	}
...
func TestReadWriteProcessMemory(t *testing.T) {
	...
	err = ReadProcessMemory(process, uintptr(unsafe.Pointer(&testBuffer[0])), &buffer[0], uintptr(len(buffer)), nil)
	if err != nil {
	...
	err = WriteProcessMemory(process, uintptr(unsafe.Pointer(&testBuffer[0])), &buffer[0], uintptr(len(buffer)), nil)
	if err != nil {
...
func TestSystemModuleVersions(t *testing.T) {
	...
	for bufferSize := uint32(128 * 1024); ; {
		...
		err := NtQuerySystemInformation(SystemModuleInformation, unsafe.Pointer(&moduleBuffer[0]), bufferSize, &bufferSize)
		switch err {
	...
	for i := range modules {
		moduleName := ByteSliceToString(modules[i].FullPathName[modules[i].OffsetToFileName:])
		driverPath := `\\?\GLOBALROOT` + ByteSliceToString(modules[i].FullPathName[:])
		var zero Handle
		infoSize, err := GetFileVersionInfoSize(driverPath, &zero)
		if err != nil {
			if err != ERROR_FILE_NOT_FOUND && err != ERROR_RESOURCE_TYPE_NOT_FOUND {
				t.Errorf("%v: %v", moduleName, err)
			}
			continue
		}
		versionInfo := make([]byte, infoSize)
		if err = GetFileVersionInfo(driverPath, 0, infoSize, unsafe.Pointer(&versionInfo[0])); err != nil {
			t.Errorf("%v: %v", moduleName, err)
			continue
		}
		var fixedInfo *VS_FIXEDFILEINFO
		fixedInfoLen := uint32(unsafe.Sizeof(*fixedInfo))
		err = VerQueryValue(unsafe.Pointer(&versionInfo[0]), `\`, (unsafe.Pointer)(&fixedInfo), &fixedInfoLen)
		if err != nil {
			t.Errorf("%v: %v", moduleName, err)
			continue
		}
...
func TestNtCreateFileAndNtSetInformationFile(t *testing.T) {
	...
	objectName, err := NewNTUnicodeString("\\??\\" + testDirPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	var testDirHandle Handle
	err = NtCreateFile(&testDirHandle, FILE_GENERIC_READ|FILE_GENERIC_WRITE, oa, &iosb,
		&allocSize, 0, FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE, FILE_OPEN,
		FILE_DIRECTORY_FILE, 0, 0)
	if err != nil {
		t.Fatalf("NtCreateFile(%v) failed: %v", testDirPath, err)
	}
	defer CloseHandle(testDirHandle)
	// Create a file in test directory with NtCreateFile.
	fileName := "filename"
	filePath := filepath.Join(testDirPath, fileName)
	objectName, err = NewNTUnicodeString(fileName)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = NtCreateFile(&fileHandle, FILE_GENERIC_READ|FILE_GENERIC_WRITE|DELETE, oa, &iosb,
		&allocSize, 0, FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE, FILE_CREATE,
		0, 0, 0)
	if err != nil {
	...
	newNameUTF16, err := UTF16FromString(newName)
	if err != nil {
		t.Fatal(err)
	}
	...
	copy((*[MAX_LONG_PATH]uint16)(unsafe.Pointer(&typedBufferPtr.FileName[0]))[:fileNameLen/2:fileNameLen/2], newNameUTF16)
	err = NtSetInformationFile(fileHandle, &iosb, &buffer[0], uint32(bufferSize), FileRenameInformation)
	if err != nil {
...
var deviceClassNetGUID = &GUID{0x4d36e972, 0xe325, 0x11ce, [8]byte{0xbf, 0xc1, 0x08, 0x00, 0x2b, 0xe1, 0x03, 0x18}}
var deviceInterfaceNetGUID = &GUID{0xcac88484, 0x7515, 0x4c03, [8]byte{0x82, 0xe6, 0x71, 0xa8, 0x7a, 0xba, 0xc3, 0x61}}

func TestListLoadedNetworkDevices(t *testing.T) {
	devInfo, err := SetupDiGetClassDevsEx(deviceClassNetGUID, "", 0, DIGCF_PRESENT, 0, "")
	if err != nil {
		t.Fatal(err)
	}
	defer devInfo.Close()
	for i := 0; ; i++ {
		...
		friendlyName, err := devInfo.DeviceRegistryProperty(devInfoData, SPDRP_DEVICEDESC)
		if err != nil {
			t.Fatal(err)
		}
		var status, problemCode uint32
		err = CM_Get_DevNode_Status(&status, &problemCode, devInfoData.DevInst, 0)
		if err != nil || (status&DN_DRIVER_LOADED|DN_STARTED) != DN_DRIVER_LOADED|DN_STARTED {
			continue
		}
		...
		interfaces, err := CM_Get_Device_Interface_List(instanceId, deviceInterfaceNetGUID, CM_GET_DEVICE_INTERFACE_LIST_PRESENT)
...
func TestListWireGuardDrivers(t *testing.T) {
	devInfo, err := SetupDiCreateDeviceInfoListEx(deviceClassNetGUID, 0, "")
	if err != nil {
		t.Fatal(err)
	}
	defer devInfo.Close()
	devInfoData, err := devInfo.CreateDeviceInfo("WireGuard", deviceClassNetGUID, "", 0, DICD_GENERATE_ID)
	if err != nil {
		t.Fatal(err)
	}
	err = devInfo.SetDeviceRegistryProperty(devInfoData, SPDRP_HARDWAREID, []byte("W\x00i\x00r\x00e\x00G\x00u\x00a\x00r\x00d\x00\x00\x00\x00\x00"))
	if err != nil {
		t.Fatal(err)
	}
	err = devInfo.BuildDriverInfoList(devInfoData, SPDIT_COMPATDRIVER)
	if err != nil {
		t.Fatal(err)
	}
	defer devInfo.DestroyDriverInfoList(devInfoData, SPDIT_COMPATDRIVER)
	for i := 0; ; i++ {
		drvInfoData, err := devInfo.EnumDriverInfo(devInfoData, SPDIT_COMPATDRIVER, i)
		if err != nil {
			if err == ERROR_NO_MORE_ITEMS {
				break
			}
...
func TestProcThreadAttributeHandleList(t *testing.T) {
	...
	executable16, err := UTF16PtrFromString(filepath.Join(system32, "cmd.exe"))
	if err != nil {
		t.Fatal(err)
	}
	args16, err := UTF16PtrFromString(ComposeCommandLine([]string{"/c", "echo " + sentinel}))
	if err != nil {
		t.Fatal(err)
	}
	attributeList, err := NewProcThreadAttributeList(1)
	if err != nil {
		t.Fatal(err)
	}
	defer attributeList.Delete()
	si := &StartupInfoEx{
		StartupInfo:             StartupInfo{Cb: uint32(unsafe.Sizeof(StartupInfoEx{}))},
	...
	func() {
		...
		handles := []Handle{Handle(pipeW.Fd())}
		attributeList.Update(PROC_THREAD_ATTRIBUTE_HANDLE_LIST, unsafe.Pointer(&handles[0]), uintptr(len(handles))*unsafe.Sizeof(handles[0]))
		si.Flags |= STARTF_USESTDHANDLES
		si.StdOutput = handles[0]
		// Go 1.16's pipe handles aren't inheritable, so mark it explicitly as such here.
		SetHandleInformation(handles[0], HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT)
	}()
	pi := new(ProcessInformation)
	err = CreateProcess(executable16, args16, nil, nil, true, CREATE_DEFAULT_ERROR_MODE|CREATE_UNICODE_ENVIRONMENT|EXTENDED_STARTUPINFO_PRESENT, nil, nil, &si.StartupInfo, pi)
	if err != nil {
		t.Fatal(err)
	}
	defer CloseHandle(pi.Thread)
	defer CloseHandle(pi.Process)
...
func TestWSALookupService(t *testing.T) {
	...
	querySet.NameSpace = NS_BTH
	querySet.Size = uint32(unsafe.Sizeof(WSAQUERYSET{}))

	var handle Handle
	err := WSALookupServiceBegin(&querySet, flags, &handle)
	if err != nil {
		if errors.Is(err, WSASERVICE_NOT_FOUND) {
			t.Skip("WSA Service not found, so skip this test")
		}
		t.Fatal(err)
	}

	defer WSALookupServiceEnd(handle)

	n := int32(unsafe.Sizeof(WSAQUERYSET{}))
	buf := make([]byte, n)
items_loop:
	for {
		q := (*WSAQUERYSET)(unsafe.Pointer(&buf[0]))
		err := WSALookupServiceNext(handle, flags, &n, q)
		switch err {
		case WSA_E_NO_MORE, WSAENOMORE:
		...
		case nil:
			...
			t.Logf("%s -> %s\n", UTF16PtrToString(q.ServiceInstanceName), addr)
