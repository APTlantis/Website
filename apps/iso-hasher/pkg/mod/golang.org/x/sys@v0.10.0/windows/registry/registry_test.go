...
func TestReadSubKeyNames(t *testing.T) {
	k, err := OpenKey(CLASSES_ROOT, "TypeLib", ENUMERATE_SUB_KEYS)
	if err != nil {
		t.Fatal(err)
	}
	defer k.Close()
...
func TestCreateOpenDeleteKey(t *testing.T) {
	k, err := OpenKey(CURRENT_USER, "Software", QUERY_VALUE)
	if err != nil {
		t.Fatal(err)
	}
	defer k.Close()
	...
	testK, exist, err := CreateKey(k, testKName, CREATE_SUB_KEY)
	if err != nil {
		t.Fatal(err)
	}
	defer testK.Close()
	...
	testKAgain, exist, err := CreateKey(k, testKName, CREATE_SUB_KEY)
	if err != nil {
		t.Fatal(err)
	}
	...
	testKOpened, err := OpenKey(k, testKName, ENUMERATE_SUB_KEYS)
	if err != nil {
		t.Fatal(err)
	}
	defer testKOpened.Close()

	err = DeleteKey(k, testKName)
	if err != nil {
		t.Fatal(err)
	}

	testKOpenedAgain, err := OpenKey(k, testKName, ENUMERATE_SUB_KEYS)
	if err == nil {
...
var ValueTests = []ValueTest{
	{Type: SZ, Name: "String1", Value: ""},
	{Type: SZ, Name: "String2", Value: "\000", WillFail: true},
	{Type: SZ, Name: "String3", Value: "Hello World"},
	{Type: SZ, Name: "String4", Value: "Hello World\000", WillFail: true},
	{Type: EXPAND_SZ, Name: "ExpString1", Value: ""},
	{Type: EXPAND_SZ, Name: "ExpString2", Value: "\000", WillFail: true},
	{Type: EXPAND_SZ, Name: "ExpString3", Value: "Hello World"},
	{Type: EXPAND_SZ, Name: "ExpString4", Value: "Hello\000World", WillFail: true},
	{Type: EXPAND_SZ, Name: "ExpString5", Value: "%PATH%"},
	{Type: EXPAND_SZ, Name: "ExpString6", Value: "%NO_SUCH_VARIABLE%"},
	{Type: EXPAND_SZ, Name: "ExpString7", Value: "%PATH%;."},
	{Type: BINARY, Name: "Binary1", Value: []byte{}},
	{Type: BINARY, Name: "Binary2", Value: []byte{1, 2, 3}},
	{Type: BINARY, Name: "Binary3", Value: []byte{3, 2, 1, 0, 1, 2, 3}},
	{Type: DWORD, Name: "Dword1", Value: uint64(0)},
	{Type: DWORD, Name: "Dword2", Value: uint64(1)},
	{Type: DWORD, Name: "Dword3", Value: uint64(0xff)},
	{Type: DWORD, Name: "Dword4", Value: uint64(0xffff)},
	{Type: QWORD, Name: "Qword1", Value: uint64(0)},
	{Type: QWORD, Name: "Qword2", Value: uint64(1)},
	{Type: QWORD, Name: "Qword3", Value: uint64(0xff)},
	{Type: QWORD, Name: "Qword4", Value: uint64(0xffff)},
	{Type: QWORD, Name: "Qword5", Value: uint64(0xffffff)},
	{Type: QWORD, Name: "Qword6", Value: uint64(0xffffffff)},
	{Type: MULTI_SZ, Name: "MultiString1", Value: []string{"a", "b", "c"}},
	{Type: MULTI_SZ, Name: "MultiString2", Value: []string{"abc", "", "cba"}},
	{Type: MULTI_SZ, Name: "MultiString3", Value: []string{""}},
	{Type: MULTI_SZ, Name: "MultiString4", Value: []string{"abcdef"}},
	{Type: MULTI_SZ, Name: "MultiString5", Value: []string{"\000"}, WillFail: true},
	{Type: MULTI_SZ, Name: "MultiString6", Value: []string{"a\000b"}, WillFail: true},
	{Type: MULTI_SZ, Name: "MultiString7", Value: []string{"ab", "\000", "cd"}, WillFail: true},
	{Type: MULTI_SZ, Name: "MultiString8", Value: []string{"\000", "cd"}, WillFail: true},
	{Type: MULTI_SZ, Name: "MultiString9", Value: []string{"ab", "\000"}, WillFail: true},
}

func setValues(t *testing.T, k Key) {
...
func enumerateValues(t *testing.T, k Key) {
...
func testGetStringValue(t *testing.T, k Key, test ValueTest) {
	...
	if gottype == EXPAND_SZ {
		_, err = ExpandString(got)
		if err != nil {
...
func testGetIntegerValue(t *testing.T, k Key, test ValueTest) {
...
func testGetBinaryValue(t *testing.T, k Key, test ValueTest) {
...
func testGetStringsValue(t *testing.T, k Key, test ValueTest) {
...
func testGetValue(t *testing.T, k Key, test ValueTest, size int) {
	if size <= 0 {
		return
	}
...
func testValues(t *testing.T, k Key) {
	for _, test := range ValueTests {
		switch test.Type {
		case SZ, EXPAND_SZ:
			if test.WillFail {
...
func testStat(t *testing.T, k Key) {
	subk, _, err := CreateKey(k, "subkey", CREATE_SUB_KEY)
	if err != nil {
		t.Error(err)
		return
	}
...
func deleteValues(t *testing.T, k Key) {
	for _, test := range ValueTests {
		if test.WillFail {
			continue
		}
...
func TestValues(t *testing.T) {
	softwareK, err := OpenKey(CURRENT_USER, "Software", QUERY_VALUE)
	if err != nil {
		t.Fatal(err)
	}
	...
	k, exist, err := CreateKey(softwareK, testKName, CREATE_SUB_KEY|QUERY_VALUE|SET_VALUE)
	if err != nil {
		t.Fatal(err)
	}
	defer k.Close()
	...
	defer DeleteKey(softwareK, testKName)
...
func TestInvalidValues(t *testing.T) {
	...
	var tests = []struct {
		...
		{DWORD, "Dword1", nil},
		{DWORD, "Dword2", []byte{1, 2, 3}},
		{QWORD, "Qword1", nil},
		{QWORD, "Qword2", []byte{1, 2, 3}},
		{QWORD, "Qword3", []byte{1, 2, 3, 4, 5, 6, 7}},
		{MULTI_SZ, "MultiString1", nil},
		{MULTI_SZ, "MultiString2", []byte{0}},
		{MULTI_SZ, "MultiString3", []byte{'a', 'b', 0}},
		{MULTI_SZ, "MultiString4", []byte{'a', 0, 0, 'b', 0}},
		{MULTI_SZ, "MultiString5", []byte{'a', 0, 0}},
	}
...
func TestGetMUIStringValue(t *testing.T) {
	if err := LoadRegLoadMUIString(); err != nil {
	...
	timezoneK, err := OpenKey(LOCAL_MACHINE,
		`SOFTWARE\Microsoft\Windows NT\CurrentVersion\Time Zones\`+tzKeyName, READ)
	if err != nil {
		t.Fatal(err)
	}
