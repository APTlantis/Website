...
func install(t *testing.T, m *Mgr, name, exepath string, c Config) {
...
func testConfig(t *testing.T, s *Service, should Config) Config {
	is, err := s.Config()
	if err != nil {
		t.Fatalf("Config failed: %s", err)
	}
...
func testRecoveryActions(t *testing.T, s *Service, should []RecoveryAction) {
...
func testResetPeriod(t *testing.T, s *Service, should uint32) {
...
func testSetRecoveryActions(t *testing.T, s *Service) {
	r := []RecoveryAction{
		{
			Type:  NoAction,
...
func testRebootMessage(t *testing.T, s *Service, should string) {
...
func testRecoveryCommand(t *testing.T, s *Service, should string) {
...
func testRecoveryActionsOnNonCrashFailures(t *testing.T, s *Service, should bool) {
...
func testMultipleRecoverySettings(t *testing.T, s *Service, rebootMsgShould, recoveryCmdShould string, actionsFlagShould bool) {
...
func testControl(t *testing.T, s *Service, c svc.Cmd, expectedErr error, expectedStatus svc.Status) {
...
func remove(t *testing.T, s *Service) {
	err := s.Delete()
	if err != nil {
...
func TestListDependentServices(t *testing.T) {
	...
	install(t, m, baseServiceName, "", Config{})
	baseService, err := m.OpenService(baseServiceName)
	if err != nil {
		t.Fatalf("OpenService failed: %v", err)
	}
	defer remove(t, baseService)
	install(t, m, dependentServiceName, "", Config{Dependencies: []string{baseServiceName}})
