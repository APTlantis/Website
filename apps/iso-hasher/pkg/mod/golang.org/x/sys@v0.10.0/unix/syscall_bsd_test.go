...
func TestSysctlRaw(t *testing.T) {
	...
	_, err := SysctlRaw("kern.proc.pid", Getpid())
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestSysctlUint32(t *testing.T) {
	maxproc, err := SysctlUint32("kern.maxproc")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSysctlClockinfo(t *testing.T) {
	ci, err := SysctlClockinfo("kern.clockrate")
	if err != nil {
		if runtime.GOOS == "openbsd" && (err == ENOMEM || err == EIO) {
			if osrev, _ := SysctlUint32("kern.osrevision"); osrev <= 202010 {
