...
func TestEpollIn(t *testing.T) {
	efd, err := EpollCreate1(0) // no CLOEXEC equivalent on z/OS
	if err != nil {
...
func TestEpollHup(t *testing.T) {
	...
	if n < 1 || events[0].Events&EPOLLHUP == 0 {
...
func TestEpollInManyFds(t *testing.T) {
	efd, err := EpollCreate1(4) // Like on Linux, size arg is ignored.
	if err != nil {
...
func TestEpollErrors(t *testing.T) {
	...
	ev := EpollEvent{Events: EPOLLIN, Fd: int32(fd)}
	if err = EpollCtl(efd+1, EPOLL_CTL_ADD, fd, &ev); err != EBADF {
		t.Errorf("EpollCtl: got %v when EpollCtl ADD called with invalid epfd, expected EBADF", err)
	}

	if err = EpollCtl(efd, EPOLL_CTL_ADD, fd, &ev); err != nil {
		t.Fatalf("EpollCtl: %v", err)
	}

	if err = EpollCtl(efd, EPOLL_CTL_MOD, -2, &ev); err != ENOENT {
