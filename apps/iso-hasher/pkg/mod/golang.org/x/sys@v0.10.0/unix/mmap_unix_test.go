...
func TestMmap(t *testing.T) {
	mmapProt := PROT_NONE
	mprotectProt := PROT_READ | PROT_WRITE
	...
	b, err := Mmap(-1, 0, Getpagesize(), mmapProt, MAP_ANON|MAP_PRIVATE)
	if err != nil {
		t.Fatalf("Mmap: %v", err)
	}
	if err := Mprotect(b, mprotectProt); err != nil {
	...
	if runtime.GOOS == "aix" {
		...
		if err := Msync(b, MS_SYNC); err != nil {
			t.Fatalf("Msync: %v", err)
		}
	}

	if err := Madvise(b, MADV_DONTNEED); err != nil {
