...
func TestXattr(t *testing.T) {
	...
	err := Setxattr(f, xattrName, []byte{}, 0)
	...
	err = Setxattr(f, xattrName, []byte(xattrDataSet), 0)
	if err != nil {
		t.Fatalf("Setxattr: %v", err)
	}

	// find size
	size, err := Listxattr(f, nil)
	if err != nil {
	...
	// find size
	size, err = Getxattr(f, xattrName, nil)
	if err != nil {
	...
	_, err = Getxattr(f, xattrName, xattrDataGet)
	if err != nil {
		t.Fatalf("Getxattr: %v", err)
	}
	...
	err = Removexattr(f, xattrName)
	if err != nil {
		t.Fatalf("Removexattr: %v", err)
	}

	n := "nonexistent"
	err = Lsetxattr(n, xattrName, []byte(xattrDataSet), 0)
	if err != ENOENT {
		t.Errorf("Lsetxattr: expected %v on non-existent file, got %v", ENOENT, err)
	}

	_, err = Lgetxattr(n, xattrName, nil)
	if err != ENOENT {
...
func TestFdXattr(t *testing.T) {
	...
	err = Fsetxattr(fd, xattrName, []byte(xattrDataSet), 0)
	if err == ENOTSUP || err == EOPNOTSUPP {
		t.Skip("filesystem does not support extended attributes, skipping test")
	} else if err != nil {
	...
	// find size
	size, err = Fgetxattr(fd, xattrName, nil)
	if err != nil {
	...
	xattrDataGet := make([]byte, size)
	_, err = Fgetxattr(fd, xattrName, xattrDataGet)
	if err != nil {
		t.Fatalf("Fgetxattr: %v", err)
	}
