...
func TestMountUnmount(t *testing.T) {
	...
	for i := 0; i < fsCount; i++ {
		err = Unmount(ByteSliceToString(buffer.fsinfo[i].Mountpoint[:]), MTM_RDWR)
		if err != nil {
			// Unmount and Mount require elevated privilege
			// If test is run without such permission, skip test
			if err == EPERM {
				t.Logf("Permission denied for Unmount. Skipping test (Errno2:  %X)", Errno2())
				return
			} else if err == EBUSY {
				continue
			} else {
			...
			available = true
			fs = ByteSliceToString(buffer.fsinfo[i].Fsname[:])
			fstype = ByteSliceToString(buffer.fsinfo[i].Fstname[:])
			mountpoint = ByteSliceToString(buffer.fsinfo[i].Mountpoint[:])
	...
	// test unmount
	buffer.header = W_Mnth{}
	fsCount, err = W_Getmntent_A((*byte)(unsafe.Pointer(&buffer)), int(unsafe.Sizeof(buffer)))
	if err != nil {
		t.Fatalf("W_Getmntent_A returns with error: %s", err.Error())
	}
	for i := 0; i < fsCount; i++ {
		if ByteSliceToString(buffer.fsinfo[i].Fsname[:]) == fs {
			t.Fatalf("File system found after unmount")
		}
	}
	// test mount
	err = Mount(fs, mountpoint, fstype, MTM_RDWR, "")
	if err != nil {
	...
	for i := 0; i < fsCount; i++ {
		if ByteSliceToString(buffer.fsinfo[i].Fsname[:]) == fs &&
			ByteSliceToString(buffer.fsinfo[i].Mountpoint[:]) == mountpoint {
			fsMounted = true
		}
	}
	if !fsMounted {
...
func TestChroot(t *testing.T) {
	...
	// If test is run without such permission, skip test
	if err == EPERM {
		t.Logf("Denied permission for Chroot. Skipping test (Errno2:  %X)", Errno2())
		return
	} else if err != nil {
...
func TestFlock(t *testing.T) {
	if os.Getenv("GO_WANT_HELPER_PROCESS") == "1" {
		...
		err = Flock(int(f.Fd()), LOCK_EX|LOCK_NB)
		...
		err = FcntlFlock(f.Fd(), F_SETLK, &flock)
		if err != nil {
