...
func TestStatvfs(t *testing.T) {
	if err := Statvfs("", nil); err == nil {
		t.Fatal(`Statvfs("") expected failure`)
	}

	statvfs := Statvfs_t{}
	if err := Statvfs("/", &statvfs); err != nil {
...
func TestBasicEventPort(t *testing.T) {
	...
	defer port.Close()
	cookie := stat.Mode()
	err = port.AssociatePath(path, stat, FILE_MODIFIED, cookie)
	if err != nil {
...
func TestEventPortErrors(t *testing.T) {
	...
	if err != ETIME {
		t.Errorf("port.GetOne(%v) returned error %v, want %v", timeout, err, ETIME)
	}
	err = port.DissociateFd(uintptr(0))
	if err == nil {
		t.Errorf("unexpected success dissociating unassociated fd")
	}
	events := make([]PortEvent, 4)
...
func TestPortEventSlices(t *testing.T) {
	...
	e := events[:n]
	for _, p := range e {
		if p.Events != FILE_DELETE {
			t.Errorf("unexpected event. got %v, expected %v", p.Events, FILE_DELETE)
		}
	}
	n, err = getRetry(t, port, events, 3, timeout)
	if err != ETIME {
		t.Errorf("unexpected error. got %v, expected %v", err, ETIME)
	}
	if n != 2 {
	...
	port.AssociateFd(r.Fd(), POLLIN, nil)
	port.AssociateFd(w.Fd(), POLLOUT, nil)
	bs := []byte{41}
	w.Write(bs)
...
func TestLifreqGetMTU(t *testing.T) {
	...
	ip_fd, err := Socket(AF_INET, SOCK_DGRAM, 0)
	if err != nil {
	...
	for link, mtu := range tc {
		...
		if err = IoctlLifreq(ip_fd, SIOCGLIFMTU, &l); err != nil {
