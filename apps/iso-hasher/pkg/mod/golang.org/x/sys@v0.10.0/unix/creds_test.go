...
func TestSCMCredentials(t *testing.T) {
	...
	for _, tt := range socketTypeTests {
		fds, err := Socketpair(AF_LOCAL, tt.socketType, 0)
		if err != nil {
			t.Fatalf("Socketpair: %v", err)
		}

		err = SetsockoptInt(fds[0], SOL_SOCKET, SO_PASSCRED, 1)
		if err != nil {
			Close(fds[0])
			Close(fds[1])
		...
		if flags != 0 && flags != MSG_CMSG_CLOEXEC {
			t.Fatalf("ReadMsgUnix flags = %#x, want 0 or %#x (MSG_CMSG_CLOEXEC)", flags, MSG_CMSG_CLOEXEC)
		}
		if n != tt.dataLen {
		...
		scm, err := ParseSocketControlMessage(oob2)
		if err != nil {
			t.Fatalf("ParseSocketControlMessage: %v", err)
		}
		newUcred, err := ParseUnixCredentials(&scm[0])
		if err != nil {
...
func TestPktInfo(t *testing.T) {
	...
	for _, test := range testcases {
		t.Run(test.network, func(t *testing.T) {
			...
			if errors.Is(err, EADDRNOTAVAIL) || errors.Is(err, EAFNOSUPPORT) {
			...
			switch test.network {
			case "udp4":
				var info4 Inet4Pktinfo
				src = net.ParseIP("127.0.0.2").To4()
				copy(info4.Spec_dst[:], src)
				pktInfo = PktInfo4(&info4)

			case "udp6":
				var info6 Inet6Pktinfo
				src = net.ParseIP("2001:0DB8::1")
				copy(info6.Addr[:], src)
				pktInfo = PktInfo6(&info6)
				...
				err = raw.Control(func(fd uintptr) {
					opErr = SetsockoptInt(int(fd), SOL_IPV6, IPV6_FREEBIND, 1)
				})
				if err != nil {
					t.Fatal("Control:", err)
				}
				if errors.Is(opErr, ENOPROTOOPT) {
	...
	for _, test := range testcases {
		t.Run(test.network, func(t *testing.T) {
			...
			err = raw.Control(func(fd uintptr) {
				switch test.network {
				case "udp4":
					opErr = SetsockoptInt(int(fd), SOL_IP, IP_RECVORIGDSTADDR, 1)
				case "udp6":
					opErr = SetsockoptInt(int(fd), SOL_IPV6, IPV6_RECVORIGDSTADDR, 1)
				}
			})
			if err != nil {
				t.Fatal("Control:", err)
			...
			conn.SetReadDeadline(time.Now().Add(100 * time.Millisecond))
			oob := make([]byte, CmsgSpace(SizeofSockaddrInet6))
			...
			sa, err := ParseOrigDstAddr(&scms[0])
			if err != nil {
