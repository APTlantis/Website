...
func _() {
	...
	var (
		_ func(int, int, int) error   = Setpriority
		_ func(int, int) (int, error) = Getpriority
	)
	const (
		_ int = PRIO_USER
		_ int = PRIO_PROCESS
...
func TestFcntlInt(t *testing.T) {
	...
	defer file.Close()
	f := file.Fd()
	flags, err := FcntlInt(f, F_GETFD, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFcntlFlock(t *testing.T) {
	...
	fd, err := Open(name, O_CREAT|O_RDWR|O_CLOEXEC, 0)
	if err != nil {
	...
	if err := FcntlFlock(uintptr(fd), F_GETLK, &flock); err != nil {
...
func TestPassFD(t *testing.T) {
	...
	scms, err := ParseSocketControlMessage(oob[:oobn])
	if err != nil {
	...
	scm := scms[0]
	gotFds, err := ParseUnixRights(&scm)
	if err != nil {
...
func TestUnixRightsRoundtrip(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		for _, fds := range testCase {
			...
			n = len(b) + CmsgLen(4*len(fds))
			b = append(b, UnixRights(fds...)...)
		}
		// Truncate b
		b = b[:n]

		scms, err := ParseSocketControlMessage(b)
		if err != nil {
			t.Fatalf("ParseSocketControlMessage: %v", err)
		}
		...
		var c int
		for len(b) > 0 {
			hdr, data, remainder, err := ParseOneSocketControlMessage(b)
			if err != nil {
...
func TestRlimit(t *testing.T) {
	var rlimit, zero Rlimit
	err := Getrlimit(RLIMIT_NOFILE, &rlimit)
	if err != nil {
	...
	err = Setrlimit(RLIMIT_NOFILE, &set)
	if err != nil {
		t.Fatalf("Setrlimit: set failed: %#v %v", set, err)
	}
	var get Rlimit
	err = Getrlimit(RLIMIT_NOFILE, &get)
	if err != nil {
	...
	err = Setrlimit(RLIMIT_NOFILE, &rlimit)
	if err != nil {
...
func TestSelect(t *testing.T) {
	for {
		n, err := Select(0, nil, nil, nil, &Timeval{Sec: 0, Usec: 0})
		if err == EINTR {
...
func compareStat_t(t *testing.T, otherStat string, st1, st2 *Stat_t) {
	if st2.Dev != st1.Dev {
...
func TestFstatat(t *testing.T) {
	...
	var st2 Stat_t
	err = Fstatat(AT_FDCWD, "file1", &st2, 0)
	if err != nil {
	...
	err = Lstat("symlink1", &st1)
	if err != nil {
		t.Fatalf("Lstat: %v", err)
	}

	err = Fstatat(AT_FDCWD, "symlink1", &st2, AT_SYMLINK_NOFOLLOW)
	if err != nil {
		t.Fatalf("Fstatat: %v", err)
	}
...
func TestFchmodat(t *testing.T) {
	...
	err = Fchmodat(AT_FDCWD, "symlink1", uint32(mode), 0)
	if err != nil {
	...
	err = Fchmodat(AT_FDCWD, "symlink1", uint32(mode), AT_SYMLINK_NOFOLLOW)
	if err != nil {
		if (runtime.GOOS == "android" || runtime.GOOS == "linux" ||
			runtime.GOOS == "solaris" || runtime.GOOS == "illumos") && err == EOPNOTSUPP {
...
func TestMkdev(t *testing.T) {
	...
	if Major(dev) != major {
		t.Errorf("Major(%#x) == %d, want %d", dev, Major(dev), major)
	}
	if Minor(dev) != minor {
		t.Errorf("Minor(%#x) == %d, want %d", dev, Minor(dev), minor)
	}
}
...
func TestRenameat(t *testing.T) {
	...
	err := Renameat(AT_FDCWD, from, AT_FDCWD, to)
	if err != nil {
...
func TestUtimesNanoAt(t *testing.T) {
	...
	err = UtimesNanoAt(AT_FDCWD, symlink, ts, AT_SYMLINK_NOFOLLOW)
	if err != nil {
		t.Fatalf("UtimesNanoAt: %v", err)
	}

	var st Stat_t
	err = Lstat(symlink, &st)
	if err != nil {
		t.Fatalf("Lstat: %v", err)
	}
...
func TestSendmsgBuffers(t *testing.T) {
	...
	go func() {
		...
		n, oobn, recvflags, _, err := RecvmsgBuffers(fds[1], bufs, nil, 0)
		if err != nil {
			t.Error(err)
			return
		}
	...
	n, err := SendmsgBuffers(fds[0], bufs, nil, nil, 0)
	if err != nil {
		t.Fatal(err)
	}
	if n != 10 {
...
func TestRecvmsgControl(t *testing.T) {
	...
	fds, err := Socketpair(AF_UNIX, SOCK_STREAM, 0)
	if err != nil {
		t.Fatal(err)
	}
	defer Close(fds[0])
	...
	go func() {
		...
		_, err = SendmsgN(fds[1], nil, rights, nil, 0)
		if err != nil {
			t.Error(err)
		}
		if _, err := Write(fds[1], []byte(payload)); err != nil {
			t.Error(err)
		}
	}()
	...
	cbuf := make([]byte, CmsgSpace(4))
	_, cn, _, _, err := Recvmsg(fds[0], nil, cbuf, 0)
	if err != nil {
		t.Fatal(err)
	}
	cbuf = cbuf[:cn]
	...
	cmsgs, err := ParseSocketControlMessage(cbuf)
	if err != nil {
		t.Fatal(err)
	}
	if len(cmsgs) != 1 {
	...
	cfds, err := ParseUnixRights(&cmsgs[0])
	if err != nil {
		t.Fatal(err)
	}
	if len(cfds) != 1 {
