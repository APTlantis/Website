...
func TestSysctlUint64(t *testing.T) {
	_, err := SysctlUint64("vm.swap_total")
	if err != nil {
		t.Fatal(err)
	}
}
...
func CapEnterTest() {
	...
	if err.(*os.PathError).Err != ECAPMODE {
...
func removeTempDir(arg string) error {
	...
	if err != nil && err.(*os.PathError).Err == ENOENT {
		return nil
	}
	return err
}
...
func OpenatTest() {
	...
	fxx, err := Openat(int(f.Fd()), "xx", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		panic(err)
	}
	Close(fxx)
	...
	b, err = CapRightsIsSet(r, []uint64{CAP_READ, CAP_LOOKUP, CAP_WRITE})
	if err != nil {
		panic(fmt.Sprintf("CapRightsIsSet failed: %s %#v", err, err))
	}
	if b {
	...
	_, err = Openat(int(f.Fd()), "xx2", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err == nil {
	...
	_, err = Openat(int(f.Fd()), "xx", os.O_RDONLY, 0666)
	if err != nil {
...
func TestCapRightsSetAndClear(t *testing.T) {
	r, err := CapRightsInit([]uint64{CAP_READ, CAP_WRITE, CAP_PDWAIT})
	if err != nil {
		t.Fatalf("CapRightsInit failed: %s", err)
	}

	err = CapRightsSet(r, []uint64{CAP_EVENT, CAP_LISTEN})
	if err != nil {
		t.Fatalf("CapRightsSet failed: %s", err)
	}

	b, err := CapRightsIsSet(r, []uint64{CAP_READ, CAP_WRITE, CAP_PDWAIT, CAP_EVENT, CAP_LISTEN})
	if err != nil {
	...
	err = CapRightsClear(r, []uint64{CAP_READ, CAP_PDWAIT})
	if err != nil {
		t.Fatalf("CapRightsClear failed: %s", err)
	}

	b, err = CapRightsIsSet(r, []uint64{CAP_WRITE, CAP_EVENT, CAP_LISTEN})
	if err != nil {
		t.Fatalf("CapRightsIsSet failed: %s", err)
	}
	if !b {
